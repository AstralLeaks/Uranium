...
public:
	...
	{
		auto dWorld = Util::FindPattern(crypt("48 89 05 ? ? ? ? 48 8B 4B 78"), true, 3);
...
public:
	...
	UObject* SpawnActorFromLong(UObject* Class, FTransform trans)
	{
		...
		auto pWorld = reinterpret_cast<UObject**>(Util::FindPattern(crypt("48 89 05 ? ? ? ? 48 8B 4B 78"), true, 3));
...
namespace Functions
{
	...
	{
		World = reinterpret_cast<UObject**>(Util::FindPattern("48 89 05 ? ? ? ? 48 8B 4B 78", true, 3));
		...
		if (Pawn) {
			gamestateFunctions->SetPlaylist(FindObject(crypt("/Game/Athena/Playlists/Playlist_DefaultSolo.Playlist_DefaultSolo")));
