...
void* ProcessEventDetour(UObject* pObject, UObject* pFunction, void* pParams)
{
    if (pObject && pFunction) {
        ...
        if (FuncName.find(crypt("ServerLoadingScreenDropped")) != std::string::npos)
        {
            ...
            gameplayAbilitiesFunctions->GrantGameplayAbility(Pawn, FindObject(crypt("BlueprintGeneratedClass /TacticalSprintGame/Gameplay/GA_Athena_TacticalSprint.GA_Athena_TacticalSprint")));
            gameplayAbilitiesFunctions->GrantGameplayAbility(Pawn, FindObject(crypt("BlueprintGeneratedClass /TacticalSprintGame/Gameplay/GA_Athena_GrantTacticalSprint.GA_Athena_GrantTacticalSprint")));
            gameplayAbilitiesFunctions->GrantGameplayAbility(Pawn, FindObject(crypt("BlueprintGeneratedClass /WallRunning/Gameplay/Abilities/GA_Player_Athena_WallRun.GA_Player_Athena_WallRun")));
...
DWORD WINAPI MainThread(LPVOID)
{
    ...
    auto pWorld = Util::FindPattern(crypt("48 89 05 ? ? ? ? 48 8B 4B 78"), true, 3);
